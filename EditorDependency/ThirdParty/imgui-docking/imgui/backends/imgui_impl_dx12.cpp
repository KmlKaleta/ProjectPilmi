...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list)
{
    ...
    // (This almost always points to ImGui::GetPlatformIO().Textures.png[] but is part of ImDrawData to allow overriding or disabling texture updates).
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // Update full texture or selected blocks. We only ever write to textures.png regions which have never been used before!
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo* init_info)
{
    ...
    io.BackendFlags |= ImGuiBackendFlags_RendererHasTextures;   // We can honor ImGuiPlatformIO::Textures.png[] requests during render.
...
bool ImGui_ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format, ID3D12DescriptorHeap* srv_descriptor_heap, D3D12_CPU_DESCRIPTOR_HANDLE font_srv_cpu_desc_handle, D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.BackendFlags &= ~ImGuiBackendFlags_RendererHasTextures; // Using legacy ImGui_ImplDX12_Init() call with 1 SRV descriptor we cannot support multiple textures.png.
