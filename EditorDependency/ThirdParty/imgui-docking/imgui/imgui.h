...
// - User may submit their own textures.png to e.g. ImGui::Image() function by passing this value.
...
//   - This can be whatever to you want it to be! read the FAQ entry about textures.png for details.
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasTextures   = 1 << 4,   // Backend Renderer supports ImTextureData requests to create/update/destroy textures.png. This enables incremental texture updates and texture reloads. See https://github.com/ocornut/imgui/blob/master/docs/BACKENDS.md for instructions on how to upgrade your custom backend.
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedLinesUseTex;     // Enable anti-aliased lines/borders using textures.png where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).
...
// The maximum line width to bake anti-aliased textures.png for. Build atlas with ImFontAtlasFlags_NoBakedLines to disable baking.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedLines        = 1 << 0,  // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f wide line or lines thin enough to be drawn using textures.png, otherwise *3 the number of triangles)
    ImDrawListFlags_AntiAliasedLinesUseTex  = 1 << 1,  // Enable anti-aliased lines/borders using textures.png when possible. Require backend to render with bilinear filtering (NOT point/nearest filtering).
...
struct ImDrawList
{
    ...
    //   In future versions we will use textures.png to provide cheaper and higher-quality circles.
...
struct ImDrawData
{
    ...
    ImVector<ImTextureData*>* Textures;     // List of textures.png to update. Most of the times the list is shared by all ImDrawData, has only 1 texture and it doesn't need any update. This almost always points to ImGui::GetPlatformIO().Textures.png[]. May be overridden or set to NULL if you want to manually update textures.png.
...
// - ImTextureID    TexID           = lower-level identifier stored in ImDrawCmd. ImDrawCmd can refer to textures.png not created by the backend, and for which there's no ImTextureData.
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoBakedLines       = 1 << 2,   // Don't build thick line textures.png into the atlas (save a little texture memory, allow support for point/nearest filtering). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};
...
//  - ImGui_ImplXXXX_RenderDrawData() functions generally iterate platform_io->Textures.png[] to create/update/destroy each ImTextureData instance.
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              Clear();                    // Clear everything (fonts + textures.png). Don't call mid-frame!
    ...
    // Output
    // - Because textures.png are dynamically created/resized, the current texture identifier may changed at *ANY TIME* during the frame.
    ...
    // [Internal]
    ImVector<ImTextureData*>    TexList;            // Texture list (most often TexList.Size == 1). TexData is always == TexList.back(). DO NOT USE DIRECTLY, USE GetDrawData().Textures.png[]/GetPlatformIO().Textures.png[] instead!
...
inline ImTextureID ImDrawCmd::GetTexID() const
{
    ...
    // - You must correctly iterate and handle ImTextureData requests stored in ImDrawData::Textures.png[]. See docs/BACKENDS.md.
...
struct ImGuiPlatformIO
{
    ...
    // Optional: Maximum texture size supported by renderer (used to adjust how we size textures.png). 0 if not known.
    ...
    // Textures.png list (the list is updated by calling ImGui::EndFrame or ImGui::Render)
    // The ImGui_ImplXXXX_RenderDrawData() function of each backend generally access this via ImDrawData::Textures.png which points to this. The array is available here mostly because backends will want to destroy textures.png on shutdown.
    ImVector<ImTextureData*>        Textures;           // List of textures.png used by Dear ImGui (most often 1) + contents of external texture list is automatically appended into this.
